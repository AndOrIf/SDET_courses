...
class Pepperoni extends Pizza implements ICookable, IDeliverable, ICalculatePrice {
    ...
    public Pepperoni(int size,  String doughType, int count) {
        super("Пепперони", 499.99);
        this.size = size;
    ...
    public void calculatePrice() { // Много if`ов (((
        try {
            ...
            if (getSize() == 25) {
                ...
                setPrice(getPrice() + 159.99);
            } else if (getSize() == 35) {
                setPrice(getPrice() + 199.99);
    ...
    @Override
    public void cook() {
        System.out.println("Готовим вашу Пепперони!");
        while (cookingTime > 0) {
